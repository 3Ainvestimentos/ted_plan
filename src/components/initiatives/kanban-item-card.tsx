...
/**
 * ============================================
 * COMPONENTE: KanbanItemCard
 * ============================================
 * 
 * Card para exibir um item no Kanban hierárquico.
 * Similar ao KanbanTaskCard mas adaptado para itens.
 * 
...
export function KanbanItemCard({ item, initiativeId, onClick, onExpand }: KanbanItemCardProps) {
  const [{ isDragging }, drag] = useDrag(() => ({
    type: 'item',
    item: { id: item.id, initiativeId }, // Passa o ID do item e da iniciativa
    collect: (monitor) => ({
  ...
  const priorityColorMapping: Record<typeof item.priority, string> = {
    'Alta': 'bg-red-100 text-red-700',
    'Média': 'bg-yellow-100 text-yellow-700',
    'Baixa': 'bg-blue-100 text-blue-700'
  };
  const priorityColorClass = priorityColorMapping[item.priority] || "bg-gray-200 text-gray-700";

  let statusIndicator = null;
  if (item.status === 'Em Risco') {
    statusIndicator = (
      <div title="Em Risco">
        <AlertTriangle className="h-4 w-4 text-orange-500 ml-auto" />
      </div>
    );
  } else if (item.status === 'Atrasado' || itemIsOverdue) {
    statusIndicator = (
      <div title="Atrasado">
  ...
  const responsible = item.responsible || 'Sem responsável';
  ...
  return (
    <div ref={drag as any} style={{ opacity: isDragging ? 0.5 : 1 }} className="cursor-grab active:cursor-grabbing">
      <Card className={cn(
        ...
        itemIsOverdue ? 'bg-red-50 border-red-200' : 'bg-card',
        item.status === 'Em Risco' ? 'border-l-4 border-l-orange-500' : '',
        (item.status === 'Atrasado' || itemIsOverdue) ? 'border-l-4 border-l-red-500' : ''
      )}>
        <CardContent className="p-3 space-y-3">
          ...
          {hasSubItems && (
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>{item.subItems?.length || 0} subitens</span>
              {onExpand && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            ...
            <div className="flex items-center gap-2">
              {item.deadline && (
                <span>{new Date(item.deadline).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}</span>
              )}
