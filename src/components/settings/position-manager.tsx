...
import { UpsertPositionModal } from "./upsert-position-modal";
...
export function PositionManager() {
    ...
    const { collaborators, isLoading, bulkUpdatePositions, updateCollaboratorHistory } = useTeamControl();
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [modalData, setModalData] = useState<{collaborator: Collaborator, position?: any} | null>(null);
    ...
    const handleDownloadTemplate = () => {
        const csvContent = "email,date,position\njohn.doe@example.com,2024-01-15,Analista Jr";
        const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
        const link = document.createElement("a");
        if (link.download !== undefined) {
            const url = URL.createObjectURL(blob);
            link.setAttribute("href", url);
            link.setAttribute("download", "template_cargos.csv");
    ...
    const handleImport = () => {
        ...
        Papa.parse(selectedFile, {
            ...
            complete: async (results) => {
                try {
                    const requiredFields = ['email', 'date', 'position'];
                    ...
                    await bulkUpdatePositions(results.data as any[]);

                    toast({
                        title: "Importação Concluída",
                        description: "Os dados de cargos e promoções foram atualizados."
                    });
    ...
    const openEditModal = (collaborator: Collaborator, position: any) => {
        setModalData({ collaborator, position });
        setIsModalOpen(true);
    };

    const handleDelete = async (collaborator: Collaborator, positionToDelete: any) => {
        const updatedHistory = collaborator.positionHistory?.filter(p => p.date !== positionToDelete.date || p.position !== positionToDelete.position) || [];
    ...
    return (
        ...
        <CardContent className="pt-6 space-y-6">
            <div>
                <h3 className="text-lg font-medium">Gerenciar Histórico de Cargos e Promoções</h3>
                <p className="text-sm text-muted-foreground">
                    Adicione ou importe o histórico de cargos dos colaboradores.
                </p>
            </div>
                    ...
                    collaborators.map(c => (
                        <div key={c.id} className="p-4 rounded-lg border">
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {c.positionHistory && c.positionHistory.length > 0 ? (
                                        c.positionHistory.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((p, i) => (
                                            <TableRow key={i}>
                                                <TableCell>{format(new Date(p.date), "dd/MM/yyyy")}</TableCell>
                                                <TableCell>{p.position}</TableCell>
                                                <TableCell className="text-right">
                                                    <Button variant="ghost" size="icon" onClick={() => openEditModal(c, p)}><Edit className="h-4 w-4"/></Button>
                                                    <AlertDialog>
                                                        ...
                                                        <AlertDialogContent>
                                                          ...
                                                          <AlertDialogFooter>
                                                            ...
                                                            <AlertDialogAction onClick={() => handleDelete(c, p)}>Remover</AlertDialogAction>
                                                          </AlertDialogFooter>
                                                        </AlertDialogContent>
                                                      </AlertDialog>
                                                </TableCell>
